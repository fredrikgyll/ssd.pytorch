...
parser.add_argument(
    '-f', default=None, type=str, help="Dummy arg so we can load in Jupyter Notebooks"
)
...
def test_net(save_folder, net, cuda, testset, transform, thresh):
    ...
    filename = f'{save_folder}test1.txt'
    ...
    for i in range(num_images):
        ...
        with open(filename, mode='a') as f:
            ...
            for box in annotation:
                cord_boxes = ' || '.join(str(b) for b in box)
                f.write(f'label: {cord_boxes}\n')
        if cuda:
            x = x.cuda()
        ...
        for i in range(detections.size(1)):
            ...
            while detections[0, i, j, 0] >= 0.6:
                ...
                pred_num += 1
                with open(filename, mode='a') as f:
                    box_cords = ' || '.join(str(c) for c in coords)
                    f.write(
                        f'{pred_num} label: {label_name} score: {score} {box_cords}\n'
                    )
                j += 1
...
def test_voc():
    ...
    testset = VOCDetection(
        args.voc_root, [('2007', 'test')], None, VOCAnnotationTransform()
    )
    if args.cuda:
        net = net.cuda()
